...
const typeCheckObject = (payloadObject, objectMap, keyChain = "") => {
  ...
  for (let i = 0; i < payloadKeys.length; i++) {
    ...
    if ((payloadValue === null || payloadValue === undefined))
      typeCheckObjectErrors += ` \n > The payload key "${key}" was given a value of null/undefined but values cannot be set to null/undefined with typeSafe on`
    else if (initialStateType === undefined)
      typeCheckObjectErrors += ` \n > The key "${key}" was set on the payload "${keyChain}" but was not defined on the initialState for "${keyChain}"`
    else {
      if (Array.isArray(payloadValue) && Array.isArray(initialStateType))
        ...
        typeCheckObjectErrors += ` \n > The payload key "${key}" was given a value with type "${payloadValueType}" but was initially set with type "${getValueType(initialStateType)}"`
  ...
  if (keyChain.length && objectMapKeys.length) {
    typeCheckObjectErrors += ` \n > You were missing the following keys on for the payload key "${keyChain}": ${objectMapKeys.join(", ")}`
  }
...
const typeCheckCanBeOneOf = (payloadKey, payloadValue, initialStateType) => {
  ...
  return ` \n > The key "${payloadKey}" was given "${payloadValue}" but "${payloadKey}" can only be set to one of the following: ${oneOfOptions.join(", ")}`
}
...
export const typeCheckArray = (payloadValue, arrayItemStructure, payloadKey) => {
  ...
  for (let i = 0; i < payloadValue.length; i++) {
    ...
    if (payloadItemType !== arrayItemType)
      return ` \n > The array for payload key "${payloadKey}" contains a value of type "${payloadItemType}" set at position ${i}. The array for key "${payloadKey}" only supports values with type "${arrayItemType}"`
    ...
    if (payloadItemType === "object") {
      ...
      if (typeCheckErrors.length)
        return ` \n > The array for payload key "${payloadKey}" supports objects, but the object set at position ${i} has the following errors: ${typeCheckErrors} \n`
    }
  }
